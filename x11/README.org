...
Currently on xmonad. Running into some issues with dbus on GuixSD, and probably should explicitly use ~dbus-launch~ anyway.
#+begin_src sh
  # exec ~/.xmonad/my-xmonad
  dbus-launch --exit-with-session ~/.xmonad/xmonad-x86_64-linux
#+end_src
